...
class Home_Hack_Model2 extends Model {
	...
	function cacheGet ($key) {
		$this->cacheConnect();
		if (!empty($this->memcache)) return $this->memcache->get($key);
		return null;
	}

	function cachePut ($key, $value) {
		$this->cacheConnect();
		if (!empty($this->memcache)) return $this->memcache->set($key, $value, null, 1800);
	}
	...
	{
		// Try cache first
		$cacheKey = 'articles-' . implode('-', $tags) . '-' . $number;
		$results = $this->cacheGet($cacheKey);
