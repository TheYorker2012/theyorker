...
<?php if($images->num_rows() > 0) foreach ($images->result() as $image) {?>
	<div style="display:inline;padding: 10px;float:left">
		<h5><?=$image->image_title?> - <a href="#" onclick="document.getElementById('image_id').value='<?=$image->image_id?>';document.getElementById('upload_form').style.display='block';return false;"><?=$image->image_id?></a></h5>
