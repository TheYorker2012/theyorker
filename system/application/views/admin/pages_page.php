		...
		<form action="<?php echo($target); ?>" method="post">
			<fieldset>
				<div>
					...
					<input name="codename" id="codename" size="35" value=<?php
						echo '"';
						if (!empty($codename)) {
						...
						?> />
					<br />
					<label id="title_label" for="head_title">Header Title:</label>
					<input name="head_title" id="head_title" size="35" value="<?php if (!empty($head_title)) { echo(xml_escape($head_title));} ?>" />
					<label for="title_separate">Separate header and body titles</label>
					<input type="checkbox" name="title_separate" id="title_separate"<?php echo($title_separate ? ' checked="checked"' : ''); ?> />
					<div id="separate_title">
						<label for="body_title">Body Title:</label>
						<input name="body_title" id="body_title" size="35" value="<?php if (!empty($body_title)) { echo(xml_escape($body_title));} ?>" />
					</div>
					
					<br />
					<label for="description">Description</label>
					<input name="description" id="description" size="35" value="<?php if (!empty($description)) { echo(xml_escape($description));} ?>" />
					<br />
					<label for="keywords">Keywords</label>
					<input name="keywords" id="keywords" size="35" value="<?php if (!empty($keywords)) { echo(xml_escape($keywords));} ?>" />
					<br />
			...
			<fieldset>
				<div>
					<input type="submit" class="button" name="save_button" id="save_button" value="Save" />
				</div>
			</fieldset>
		</form>
	</div>
...
?>
		...
		<form action="<?php echo($target); ?>" method="post">
			<fieldset>
				...
				?>
					...
					<textarea name="<?php echo($property['id']);?>" class="full" rows="10" cols="70" <?php if (!$permissions['prop_edit']) {echo 'readonly="readonly"';} ?>><?php echo(xml_escape($property['text']));?></textarea>
					<br />
				<?php
				}
	...
	<?php } ?>
					...
					<textarea name="newprop" class="full" rows="10" cols="70"></textarea>
					<br />
				</div>
				<?php
