	...
	<div class='BlueBox' style='width: 100%;'>
		...
		<div id='container<?php echo $entry['id']; ?>' class='feedback'>
			<div class='top'>
				<span class='right'>
					<?php echo $entry['time']; ?>
					<?php if ($entry['email'] != '') { ?>
						<br /><a href='mailto:<?php echo $entry['email']; ?>'><span class='orange'><?php echo xml_escape($entry['email']); ?></span></a>
					<?php } ?>
				</span>
				<span class='bold'>
			...
			<div id='feedback<?php echo $entry['id']; ?>' class='main'>
				<?php echo nl2br(xml_escape($entry['comment'])); ?>
				<br />
		...
		<script type='text/javascript'>
		document.getElementById('feedback<?php echo $entry['id']; ?>').style.display = 'none';
		</script>
		<?php } ?>
	</div>