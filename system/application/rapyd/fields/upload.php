...
class uploadField extends objField{
  ...
  function draw_preview_link(){
    ...
    if ( file_exists($this->serverPath($this->upload_root)."/thumb_".$this->value)){
        $webPath = $this->www_path."thumb_".$this->value;
    } else {
        $webPath = $this->www_path.$this->value;    
    }
    return  '<a onclick="'.$action.'" href="javascript:void(0);"><img src="'.$webPath.'" '.$tagParams.' width="'.$width.'" height="'.$height.'" border="0" /></a>';
  }
  ...
  function execUnlink(){
     ...
     @unlink($this->serverPath($this->upload_root)."/"."thumb_".$filename);
  }  
      ...
      case "disabled":  
        ...
        if ( (!isset($this->value)) || ($this->value == "") ){
          ...
          if (count($this->preview)>0){
            $output = $this->drawPreviewLink();
          } else {
            $output = $this->draw_upload_link();
          }
        }
        break;
