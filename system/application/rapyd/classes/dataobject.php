  ...
  */
  function _exec_post_process_functions($action){
    if (isset($this->_post_process_functions[$action])){
      ...
      $arr_values = $function["arr_values"];
      $arr_values = (count($arr_values)>0)?array_unshift($arr_values, $this):array($this);
      return call_user_func_array(array(&$this->ci, $function["name"]), $arr_values);       
    }
  }
...
/**
  ...
  * @return   mixed  value of a field or null if not set
  */
  function get_related($field_fk){
    if (isset($this->data_rel[$field_fk])) {
      return $this->data_rel[$field_fk];
    } else {
      return null;
    } 
  }
